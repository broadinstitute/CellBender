...
def comprehensive_random_seed(seed, use_cuda=False):
...
def generate_sample_dataset(
    ...
    comprehensive_random_seed(seed=random_seed, use_cuda=torch.cuda.is_available())
...
def generate_sample_dirichlet_dataset(
    ...
    if chi is None:
        ...
        for i in range(chi.shape[0]):
            chi[i, :] = draw_random_chi(rng=rng, alpha=dirichlet_alpha, n_genes=n_genes)
            if i > 0:
...
def draw_random_chi(rng: np.random.RandomState,
                    alpha: float = 1.,
                    n_genes: int = 10000) -> np.ndarray:
    ...
    chi = rng.dirichlet(alpha * np.ones(n_genes), size=1).squeeze()

    return chi


def sample_from_model(num: int,
...
def write_simulated_truth_to_h5(output_file: str,
    ...
    extra_latents = latents - {'gene_names', 'genome', 'barcodes', 'counts_true',
                               'droplet_labels', 'd_cell', 'epsilon', 'rho',
