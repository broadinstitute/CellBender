...
def generate_sample_dirichlet_dataset(
    ...
    if isinstance(dirichlet_alpha, np.ndarray):
...
def draw_random_chi(alpha: float | np.ndarray = 1.0, n_genes: int = 10000) -> torch.Tensor:
    ...
    chi = (
        torch.distributions.Dirichlet(torch.as_tensor(alpha, dtype=torch.float32) * torch.ones(n_genes).to(DEVICE))
        .sample()
        .squeeze()
    )
...
def sample_from_inferred_model(
    ...
    params = {
        name: (nodes[name]["value"].unconstrained() if (nodes[name]["type"] == "param") else nodes[name]["value"])
...
def get_dataset_dict_as_anndata(sample_dataset: Dict[str, Any]) -> "anndata.AnnData":
...
def write_simulated_data_to_h5(output_file: str, d: Dict[str, Any], cellranger_version: int = 3) -> bool:
...
def write_simulated_truth_to_h5(output_file: str, d: Dict[str, Any], cellranger_version: int = 3) -> bool:
    """Helper function to write the full (noisy) simulate dataset to an H5 file.
