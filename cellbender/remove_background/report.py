...
from cellbender.remove_background.summary import pca_2d
...
def generate_summary_plots(
    ...
    adata.var[f"n_{input_layer_key}_cells"] = np.array(
        adata.layers[input_layer_key][cells.values].sum(axis=0)
    ).squeeze()
    adata.var[f"n_{out_key}_cells"] = np.array(adata.layers[out_key][cells.values].sum(axis=0)).squeeze()
    adata.var["n_removed_cells"] = adata.var[f"n_{input_layer_key}_cells"] - adata.var[f"n_{out_key}_cells"]
    ...
    try:
        ...
        display(
            Markdown(
                "Skipping assessment over overall count removal. Presumably "
    ...
    if dev_mode:
        ...
        display(
            Markdown(
                "Typically we see that some of the low-count cells have "
    ...
    if out_key == "cellbender":
        ...
        display(Markdown("The inferred posterior probability that each droplet is non-empty."))
        display(
            Markdown(
                '*<span style="color:gray">We sometimes write "non-empty" '
    ...
    if dev_mode and (truth_file is not None):
        ...
        plt.plot([0, adata.var["n_truth"].max()], [0, adata.var["n_truth"].max()], color="lightgray", alpha=0.5)
        ...
        plt.plot([0, adata.var["n_truth"].max()], [0, 0], color="lightgray", alpha=0.5)
        plt.xlabel("True counts per gene")
        plt.ylabel(f"{out_key}: residual count ratio per gene\n({out_key} - truth) / truth")
        plt.tight_layout()
        plt.show()

        adata.var["fraction_remaining_cells_truth"] = adata.var["n_truth"] / (
            adata.var[f"n_{input_layer_key}_cells"] + 1e-5
        )
        ...
        display(Markdown("Now showing the truth in addition to the `remove-background` output."))
        ...
        display(
            Markdown(
                "This is a strange but somewhat fun way to visualize what "
    ...
    if truth_file is not None:
        ...
        display(Markdown("Here we take a look at removal of noise counts from cell containing droplets only."))
...
def assess_overall_count_removal(adata, raw_full_adata, input_layer_key="raw", out_key="cellbender"):
    ...
    empty_log_counts = np.array(
        raw_full_adata.X[[bc not in adata.obs_names for bc in raw_full_adata.obs_names]].sum(axis=1)
    ).squeeze()
    ...
    plt.hist(
        empty_log_counts.tolist() + log_counts[~cells].tolist(), histtype="step", label="empty droplets", bins=bins
    )
    plt.hist(log_counts[cells], histtype="step", label="non-empty droplets", bins=bins)
    _xx = np.linspace(plt.gca().get_xlim()[0], plt.gca().get_xlim()[-1], 100)
    ...
    plt.xticks(plt.gca().get_xticks(), [f"{n:.0f}" for n in np.power(10, plt.gca().get_xticks())], rotation=90)
    ...
    print(f"roughly {estimated_ambient_per_droplet * cells.sum():.0f} noise counts should be in non-empty droplets")
    print(
        f"that is approximately {expected_fraction_removed_from_cells * 100:.2f}% of the counts in non-empty droplets"
    )

    if not cohort_mode:
        expected_percentage = (expected_fraction_removed_from_cells + fpr) * 100
        print(
            f"with a false positive rate [FPR] of {fpr * 100}%, we would expect to remove about "
            f"{expected_percentage:.2f}% of the counts in non-empty droplets"
        )
    else:
        expected_percentage = expected_fraction_removed_from_cells * 100
        print(
            f"ran in cohort mode, so no false positive rate [FPR] target was set, "
    ...
    if np.abs(expected_percentage - removed_percentage) <= 0.5:
        ...
        if removed_percentage < expected_percentage:
            display(
                Markdown(
                    "The algorithm removed a bit less than naive expectations "
            ...
            display(
                Markdown(
                    "The algorithm removed a bit more than naive expectations "
...
def assess_learning_curve(
    ...
    large_spikes_in_train = np.any(
        (adata.uns["learning_curve_train_elbo"][46:] - adata.uns["learning_curve_train_elbo"][45:-1])
    ...
    large_deviation_in_train = np.any(
        second_half_train_elbo < np.median(second_half_train_elbo) - train_elbo_range * deviation_size
    )
    ...
    non_monotonicity = (
        (adata.uns["learning_curve_train_elbo"][46:] - adata.uns["learning_curve_train_elbo"][45:-1])
        < -3 * typical_end_variation
    ).sum() / len(adata.uns["learning_curve_train_elbo"])
    ...
    windowsize = 20
    tracking_trace = windowed_cumsum(
        adata.uns["learning_curve_train_elbo"][1:] - adata.uns["learning_curve_train_elbo"][:-1], n=windowsize
    )
    big_dip = -1 * (adata.uns["learning_curve_train_elbo"][-1] - adata.uns["learning_curve_train_elbo"][5]) / 10
    ...
    runaway_test = (
        adata.uns["learning_curve_test_elbo"][-1]
    ...
    non_convergence = (
        np.mean(
            [
                adata.uns["learning_curve_train_elbo"][-1] - adata.uns["learning_curve_train_elbo"][-2],
                adata.uns["learning_curve_train_elbo"][-2] - adata.uns["learning_curve_train_elbo"][-3],
            ]
        )
    ...
    if large_spikes_in_train or large_deviation_in_train:
        display(
            Markdown(
                "This is unusual behavior, and a reduced --learning-rate "
        ...
        display(
            Markdown(
                "This is slightly unusual behavior, and a reduced "
        ...
        display(
            Markdown(
                "This is slightly unusual behavior, and more training "
...
def plot_learning_curve(adata):
    ...
    def _mkplot():
        plt.plot(adata.uns["learning_curve_train_epoch"], adata.uns["learning_curve_train_elbo"], label="train")
        try:
            plt.plot(adata.uns["learning_curve_test_epoch"], adata.uns["learning_curve_test_elbo"], ".:", label="test")
            plt.legend()
        except Exception:
            pass
    ...
    if len(adata.uns["learning_curve_train_elbo"]) > 20:
        ...
        if (len(adata.uns["learning_curve_train_elbo"]) > 0) and (len(adata.uns["learning_curve_test_elbo"]) > 0):
            high = max(adata.uns["learning_curve_train_elbo"].max(), adata.uns["learning_curve_test_elbo"].max())
        else:
...
def assess_count_removal_per_gene(adata, raw_full_adata, input_layer_key="raw", r_squared_cutoff=0.5, extended=True):
    ...
    clims = [
        adata.obs[f"n_{input_layer_key}"][~cells].mean() / 2,
    ...
    if "approximate_ambient_profile" in adata.uns.keys():
        ...
        if empty_count_matrix.shape[0] > 100:
            approximate_ambient_profile = np.array(
                raw_full_adata[(counts > clims[0]) & (counts < clims[1])].X.mean(axis=0)
            ).squeeze()
        else:
            # a very rare edge case I've seen once
            display(
                Markdown(
                    "Having some trouble finding the empty droplets via heuristics. "
    ...
    r_squared_result = scipy.stats.pearsonr(np.log(approximate_ambient_profile[logic]), np.log(y[logic]))
    ...
    if r_squared > r_squared_cutoff:
        ...
        display(Markdown("WARNING: This deviates from expectations, and may indicate that the run did not go well"))
    ...
    display(
        HTML(
            df[df[f"n_{input_layer_key}"] > genecount_lowlim]
    ...
    if extended:
        plt.figure(figsize=(12, 4))
        plt.subplot(1, 2, 1)
        plt.semilogx(adata.var[f"n_{input_layer_key}"], adata.var["fraction_remaining"], "k.", ms=1)
        plt.ylim([-0.05, 1.05])
        plt.xlabel("Number of counts in raw data")
        plt.ylabel("Fraction of counts remaining")
...
def plot_validation_plots(adata, input_layer_key="raw", output_layer_key="cellbender", extended=True):

    display(
        Markdown(
            '*<span style="color:gray">The intent is to change the input '
    ...
    display(Markdown("The plots here show data for inferred cell-containing droplets, and exclude the empty droplets."))
    ...
    plt.subplot(1, 2, 1)
    plt.loglog(
        adata.obs[f"n_{input_layer_key}"][cells].values,
    ...
    minmax = [adata.obs[f"n_{input_layer_key}"][cells].min() / 10, adata.obs[f"n_{input_layer_key}"][cells].max() * 10]
    ...
    minmax = [adata.var[f"n_{input_layer_key}_cells"].min() / 10, adata.var[f"n_{input_layer_key}_cells"].max() * 10]
    plt.loglog(minmax, minmax, lw=1, color="gray", alpha=0.5)
    ...
    if extended:
        ...
        plt.subplot(1, 2, 1)
        plt.loglog(
            fano(adata.layers[input_layer_key][cells], axis=1),
            fano(adata.layers[output_layer_key][cells], axis=1),
            ".",
            ms=1,
        )
        ...
        plt.subplot(1, 2, 2)
        plt.loglog(
            fano(adata.layers[input_layer_key][cells], axis=0),
            fano(adata.layers[output_layer_key][cells], axis=0),
            ".",
            ms=1,
        )
        plt.loglog([1e0, 1e3], [1e0, 1e3], lw=1, color="gray", alpha=0.5)
        ...
        for bc in np.random.permutation(np.where(cells)[0])[:300]:
            cosine_dist.append(
                cosine(
                    np.array(adata.layers[input_layer_key][bc, :].todense()).squeeze(),
        ...
        display(
            Markdown(
                "We want this cosine distance to be as small as it can be "
...
def plot_gene_removal_histograms(adata, input_layer_key="raw", plot_truth=False, out_layer_key="cellbender"):
    ...
    for i, g_ind in enumerate([0, 5, 10, 20, 100, 1000]):
        ...
        plt.subplot(2, 3, i + 1)
        plt.hist(
            np.array(adata.layers[input_layer_key][:, order_gene[g_ind]].todense()).squeeze(),
            bins=bins,
            log=True,
            label=input_layer_key,
            histtype="step",
        )
        plt.hist(
            np.array(adata.layers[out_layer_key][:, order_gene[g_ind]].todense()).squeeze(),
            bins=bins,
            log=True,
            label=out_layer_key,
            alpha=0.75,
        ...
        if plot_truth:
            plt.hist(
                np.array(adata.layers["truth"][:, order_gene[g_ind]].todense()).squeeze(),
                bins=bins,
                log=True,
                label="truth",
                alpha=0.5,
...
def show_gene_expression_before_and_after(adata, input_layer_key: str = "raw", num: int = 10):
    ...
    for i in inds:
        ...
        true_ambient = np.resize(adata.var["truth_ambient_expression"][order][nz_inds], imsize[0] * imsize[1]).reshape(
            imsize
        )
        true_ambient[-1, -overflow:] = 0.0
...
def plot_gene_expression_pca(adata, key="cellbender_embedding", input_layer_key="raw", extended=True):
    ...
    display(
        Markdown(
            '*<span style="color:gray">We are not looking for anything '
    ...
    if extended:
        ...
        def _pca_color(g: int, layer):
            ...
            s = plt.scatter(
                x=adata.obsm["X_pca"][:, 0][cells][order],
        ...
        if "feature_type" in adata.var.keys():
            ...
            feature_logic = True
        features = (
            adata.var[feature_logic & (adata.var["n_cellbender"] > genecount_lowlim)]
            .sort_values(by="fraction_removed", ascending=False)
            .groupby("genome")
            .head(2)
            .index.values.tolist()
        ...
        display(
            Markdown(
                '*<span style="color:gray">We typically see selective '
...
def cluster_cells(adata, embedding_key="cellbender_embedding", n=2):
    ...
    if "truth_cell_probability" in adata.obs.keys():
        ...
        n_cell_labels = np.array([k.startswith("truth_gene_expression_cell_label_") for k in adata.var.keys()]).sum()
    else:
        n_cell_labels = n
    spec = SpectralClustering(n_clusters=n_cell_labels, random_state=0).fit_predict(adata.obsm[embedding_key][cells, :])
...
def cluster_and_compare_expression_to_truth(adata, embedding_key="cellbender_embedding"):
    ...
    display(
        Markdown(
            "Here we find cluster labels *de novo* using spectral clustering, "
    ...
    for k in np.unique(adata.obs["cluster"][cells]):
        plt.plot(
            adata.obsm["X_pca"][adata.obs["cluster"] == k, 0],
    ...
    display(
        Markdown(
            "We can now compare the gene expression profile of cells "
    ...
    for k in adata.obs["cluster"].unique():
        ...
        summmed_expression = np.array(adata.layers["cellbender"][adata.obs["cluster"] == k, :].sum(axis=0)).squeeze()
    ...
    def _gridplot(adata, upper_lim=1e-1, scale="linear"):
        ...
        for k in adata.obs["cluster"].unique():
            for j in adata.obs["cluster"].unique():
                plt.subplot(n_clusters, n_clusters, n_clusters * k + j + 1)
    ...
    if dist.max() > 0.9:
        plt.imshow(dist, vmin=0, vmax=dist[dist < 0.9].max() + 0.1, cmap=plt.cm.gray_r)
    else:
        plt.imshow(dist, vmin=0, cmap=plt.cm.gray_r)
...
def compare_latents(adata, input_layer_key="raw"):
    ...
    if not truth_exists:
        display(
            Markdown(
                "This section deals with particular details of the "
    ...
    if truth_exists:
        plt.figure(figsize=(12, 3))
        plt.plot(adata.var["ambient_expression"].values - adata.var["truth_ambient_expression"].values)
    ...
    if rho is None:
        display(
            Markdown(
                'Swapping fraction not inferred.  `--model` may have been "ambient" instead of "swapping" or "full"'
            )
        )
    else:
        ...
        if truth_exists:
            plt.hist(adata.obs["truth_swapping_fraction"].values, bins=xx, label="Truth", alpha=0.5)
        plt.plot(
            xx,
            (adata.shape[0] / nbins * beta.pdf(xx, consts.RHO_ALPHA_PRIOR, consts.RHO_BETA_PRIOR)),
            label="Prior",
            color="k",
        )
        plt.plot(xx, (adata.shape[0] / nbins * beta.pdf(xx, rho[0], rho[1])), label="Inferred", color="r")
    ...
    if truth_exists:
        ...
        for i, key in enumerate(keys):
            ...
            plt.scatter(
                adata.obs[f"truth_{key}"], adata.obs[key], c=adata.obs[otherkey], s=5, cmap="coolwarm", alpha=0.5
            )
            cbar = plt.colorbar()
        ...
        for i, key in enumerate(keys):
            ...
            for c in sorted(adata.obs["truth_cell_label"][cells].unique()):
                ...
                plt.plot(
                    adata.obs[f"truth_{key}"][cells & ctype],
                    adata.obs[key][cells & ctype],
                    ".",
                    label=f"{c} ({ctype.sum()} cells)",
                    alpha=0.5,
                )
            xx = [adata.obs[f"truth_{key}"][cells].min(), adata.obs[f"truth_{key}"][cells].max()]
            plt.plot(xx, xx, color="lightgray")
    ...
    plt.gca().twinx()
    if truth_exists:
        plt.plot(adata.obs["truth_droplet_efficiency"].values[order], label="truth", alpha=0.5)
    plt.plot(adata.obs["droplet_efficiency"].values[order], "r", label="inferred", alpha=0.5)
    ...
    if truth_exists:
        ...
        yy, _ = np.histogram(
            gamma(adata.uns["truth_epsilon_param"], scale=1.0 / adata.uns["truth_epsilon_param"], size=[50000]),
            bins=xx,
        )
    ...
    yy, _ = np.histogram(gamma(rh, scale=1.0 / rh, size=[50000]), bins=xx)
    ...
    plt.gca().twinx()
    if truth_exists:
        plt.semilogy(adata.obs["truth_cell_size"].values[cells][cell_order], label="truth", alpha=0.5)
    plt.semilogy(adata.obs["cell_size"].values[cells][cell_order], "r", label="inferred", alpha=0.5)
    plt.ylabel("$d$", fontsize=18, color="r")
    plt.legend()
    plt.show()

    display(Markdown(r"### The combination $d \epsilon$"))
    display(Markdown("It is easier for the model to nail this down, since there is less degeneracy."))
    ...
    if truth_exists:
        plt.semilogy(
            (
                adata.obs["truth_cell_size"].values[cells][cell_order]
    ...
    plt.semilogy(
        (adata.obs["cell_size"].values[cells][cell_order] * adata.obs["droplet_efficiency"].values[cells][cell_order]),
        "r",
        label="inferred",
        alpha=0.5,
    )
    ...
    if truth_exists:
        ...
        for c in sorted(adata.obs["truth_cell_label"][cells].unique()):
            ctype = adata.obs["truth_cell_label"] == c
            plt.semilogy(
                adata.obs["truth_droplet_efficiency"][ctype], adata.obs["truth_cell_size"][ctype], ".", alpha=0.5
            )
        ...
        for c in sorted(adata.obs["truth_cell_label"][cells].unique()):
            ctype = adata.obs["truth_cell_label"] == c
            plt.semilogy(
                adata.obs["droplet_efficiency"][ctype],
        ...
        plt.figure(figsize=(4, 4))
        plt.semilogy(adata.obs["droplet_efficiency"][cells], adata.obs["cell_size"][cells], "k.", ms=1, alpha=0.5)
    plt.ylabel("$d$", fontsize=18)
    plt.xlabel(r"$\epsilon$", fontsize=18)
    plt.title("Inferred")
    plt.tight_layout()
...
def mixed_species_plots(
    adata, input_layer_key="raw", output_layer_key="cellbender", ngene_cutoff=5000, count_cutoff=500
):
    ...
    display(
        Markdown(
            "There are multiple genomes in the dataset, so we can "
        ...
        if "feature_type" in adata.var.keys():
            if "Gene Expression" in adata.var["feature_type"].values:
                var_subset = adata.var[
                    (adata.var["genome"] == genome) & (adata.var["feature_type"] == "Gene Expression")
                ]
            else:
                var_subset = adata.var[(adata.var["genome"] == genome)]
        else:
            var_subset = adata.var[(adata.var["genome"] == genome)]
        print(f'Genome "{genome}" has {len(var_subset)} genes: {", ".join(var_subset.index.values[:3])} ...')
    ...
    for i, genome1 in enumerate(genomes):
        for genome2 in genomes[(i + 1) :]:
            ...
            for k in [input_layer_key, output_layer_key]:
                ...
                display(Markdown(f'Mean "contamination" per droplet in {k} data: {contam.mean() * 100:.2f} %'))
                display(Markdown(f'Median "contamination" per droplet in {k} data: {np.median(contam) * 100:.2f} %'))
            plt.xscale("log")
            plt.yscale("log")
...
def cell_roc_count_roc(
    output_csr: sp.csr_matrix,
    ...
    # count breakdown
    tp = np.array(noise_counts.minimum(removed_counts).sum(axis=1)).squeeze()
    ...
    display(
        Markdown(
            "Take a look at the statistics of count removal, with the "
    ...
    plot_roc_points(
        ...
        tn,
        fn,
        point_colors=np.array(truth_cell_labels)[cell_truth],
        title="Background removal from cells",
        color="black",
        show=False,
    )
    plt.show()
    ...
    print(f"Cell background removal TPR = {(tp / (tp + fn)).mean():.3f} +/- {(tp / (tp + fn)).std():.3f}")
    print(f"Cell background removal FPR = {1 - (tn / (tn + fp)).mean():.3f} +/- {(tn / (tn + fp)).std():.3f}")

    display(Markdown("And finally one more way to quanitify the amount of noise remaining:"))

    contam_cell_raw = np.array(noise_counts.sum(axis=1)).squeeze() / np.array(real_counts.sum(axis=1)).squeeze()
    contam_cell_after = np.array(noise_remaining.sum(axis=1)).squeeze() / np.array(real_counts.sum(axis=1)).squeeze()
    print(f"Per cell contamination fraction raw = {contam_cell_raw.mean():.3f} +/- {contam_cell_raw.std():.3f}")
    print(f"Per cell contamination fraction after = {contam_cell_after.mean():.3f} +/- {contam_cell_after.std():.3f}")
...
def plot_roc_points(
    ...
    **kwargs,
    ...
    for c in unique_colors:
        plt.plot((1 - specificity)[point_colors == c], sensitivity[point_colors == c], ".", ms=1, alpha=0.5, label=c)
    lgnd = plt.legend(loc="center left", bbox_to_anchor=(1, 0.5))
    _set_legend_dot_size(lgnd, size=15)
    # midpoint
    plt.plot(1 - spec_mean, sens_mean, "o", **kwargs)
    # errorbars
    plt.plot([1 - (spec_mean - spec_stdv), 1 - (spec_mean + spec_stdv)], [sens_mean, sens_mean], **kwargs)
